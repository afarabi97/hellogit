...
CONF_FILE = "/etc/classification_banner"
...
""" % bgcolor
        ...
        # Create the Right-Justified Vertical Box
        self.vbox_esc_right = Gtk.VBox()
        self.esc_label = Gtk.Label(
            label="<span font_family='liberation-sans' weight='normal' foreground='%s' size='xx-small'> (ESC to hide temporarily) </span>" %
            (fgcolor))
...
class DisplayBanner:
    ...
    def configure(self):
        ...
        parser.add_argument(
            "-x",
            "--hres",
            default=defaults["horizontal_resolution"],
            type=int,
            help="Set the Horizontal Screen Resolution")
        parser.add_argument(
            "-y",
            "--vres",
            default=defaults["vertical_resolution"],
            type=int,
        ...
        parser.add_argument(
            "--face", default=defaults["face"], help="Font type")
        parser.add_argument(
