...
import { AgentBuilderForm, BuildState } from './agent-builder-form';
...
export class AgentBuilderComponent implements OnInit {  
  ...
  onSubmit(){
    ...
    payload['id'] = "";
    //If an existing sensor is selected, replace the value with its id
    ...
    this.agentBuilderForm.build_state = BuildState.Building
    this.agentBuilderSrv.getAgentInstaller(payload)
      .subscribe(
        installer_response => {
          this.agentBuilderForm.build_state = BuildState.Built
        ...
        err => {
            this.agentBuilderForm.build_state = BuildState.Failed
  ...
  onChanges() {
        this.agentBuilderForm.valueChanges.subscribe(val => {
            this.agentBuilderForm.build_state = this.agentBuilderForm.BuildState.Unbuilt
            this.agentBuilderForm.notification_text = ''
        });
    }
