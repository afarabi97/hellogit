...
def _check_job_states(items: Dict) -> bool:
    """
    Check Job states
...
def wait_for_jobs_to_complete(master_srv: NodeSettings, timeout_minutes: int=10):
    """
    ...
    future_time = datetime.utcnow() + timedelta(minutes=timeout_minutes)
    with KubernetesWrapper(username, password, ip_address) as api:
        logging.info("Checking for active Kubernetes Jobs")
        while True:
            if future_time <= datetime.utcnow():
            ...
            try:
                ...
                if len(items) == 0 or _check_job_states(items):
                    logging.info("All Kubernetes Jobs completed successfully")
                    break
            except MaxRetryError as e:
                logging.warn(e)
