...
if [ -r /.firstboot.tmp ]; then
    ...
    if [ ! -f /var/www/MISP/app/Config/config.php ]; then
        ...
        tar xzpf /root/MISPconfig.tgz
        rm /root/MISPconfig.tgz
    fi
    ...
    if [ -z $MISP_EXTERNAL_CONFIG ] || [ $MISP_EXTERNAL_CONFIG != "true" ]; then
            echo "INFO - Not using external config for MISP"
    else
        echo "INFO - Enabled external config files for MISP"
        CONFIG_FILE=/var/www/MISP/app/Config/config.external.php
        if [ -f "$CONFIG_FILE" ]; then
            echo "INFO - Using external config file for MISP"
            echo "include 'config.external.php';" >> config.php
            echo "" >> config.php
        fi
        BOOTSTRAP_FILE=/var/www/MISP/app/Config/bootstrap.external.php
