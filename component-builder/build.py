...
class Builder:
    ...
    def _process_template(self, template_name: str, dst: str = "helm"):
        ...
        rendered_template = template.render(
            context=self.context, component=self.component
        )
    ...
    def _publish_helm(self):
        ...
        response = requests.put(
            url, data=open(helm_file, "rb"), auth=self._nexus_auth, verify=False
        )
        if response.status_code == 400:
            ...
            eprint(
                "Failed with for unknown reason with status code {}.".format(
    ...
    def build_docker_file(self):
        ...
        ret_code = run_command(
            f"docker build -t {image_path} -f {self.component_dir}/docker/Dockerfile {self.component_dir}/docker/"
        )
        if ret_code != 0:
            exit(ret_code)
    ...
    def _helm_component_rpm_command(
        self, rpm_version: str, rpm_release_num: str, containers: str
    ) -> str:
        rpm_name = self.component["name"]
        rpm_requirements = self.component["rpm_requirements"]
        return (
            'rpmbuild -bb --define="rpm_version {rpm_version}" '
            ...
            "component-builder/common/rpm/helm.spec"
        ).format(
    ...
    def _common_component_rpm_command(
        self, rpm_version: str, rpm_release_num: str, containers: str
    ) -> str:
        rpm_requirements = self.component["rpm_requirements"]
        rpm_name = self.component["name"]
        ...
        return (
            ...
            '--define="_binary_payload w2T16.xzdio" '
            '--define="rpm_name {rpm_name}" '
            "component-builder/common/rpm/common.spec"
        ).format(
            rpm_version=rpm_version,
            containers=containers,
            current_dir=PROJECT_DIR,
        ...
        try:
            if len(self.component["containers"]) > 0:
                for container in self.component["containers"]:
                    ret_code = run_command(
                        "docker pull {}".format(container), PROJECT_DIR
                    )
                    if ret_code != 0:
                        exit(ret_code)
                    ...
                    ret_code = run_command(
                        "docker save -o {name}-{version}.tar {docker_container}".format(
                            name=name, version=version, docker_container=container
                        ),
                        PROJECT_DIR,
                    )
                    if ret_code != 0:
    ...
    def _publish_rpm(self, rpm_name: str, nexus_url: str):
        ...
        response = requests.put(
            url, data=open(rpm_name, "rb"), auth=self._nexus_auth, verify=False
        )
        if response.status_code == 400:
            ...
            eprint(
                "{} for unknown reason with status code {}.".format(
    ...
    def build_rpm(self, rpm_version: str, rpm_release_num: str, nexus_url: str):
        ...
        if not self._is_common_component:
            ...
            cmd = self._helm_component_rpm_command(
                rpm_version, rpm_release_num, containers
            )
        else:
            print(
                "HELM chart not found.  The build is now assuming this is a common component build."
            )
            cmd = self._common_component_rpm_command(
                rpm_version, rpm_release_num, containers
            )
        ...
        ret_code = run_command(
            "mv /root/rpmbuild/RPMS/x86_64/tfplenum-*.rpm .", PROJECT_DIR
        )
        if ret_code != 0:
            exit(ret_code)

        rpm_name = "tfplenum-{}-{}-{}.el8.x86_64.rpm".format(
            self.component["name"], rpm_version, rpm_release_num
        )
...
def main():
    ...
    parser = ArgumentParser(
        description="This application is used to build the various HELM chart components."
    )
