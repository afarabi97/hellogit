...
docker run --ulimit nofile=1024 -v "$(pwd)/../../../":/repo/ --rm docker.nexus.sil.lab/tfplenum/vommit:0.2 gitlint --extra-path /root/rules -vv --commit "$(git log -1 --pretty=%H)" || exit_code=$?; if [ $exit_code -ne 0 ]; then echo -e "\nThe Total Errors: $exit_code\n"; exit 1; else echo -e "GITLINT PASSED\n"; fi;
...
# docker run --ulimit nofile=1024 -v "$(pwd)/../../../":/repo/ --rm docker.nexus.sil.lab/tfplenum/vommit:0.2 gitlint --extra-path /root/rules -vv --commit "99bdd2859ea8e26d9d02608b77ec1007f2bd8fb4" || exit_code=$?; if [ $exit_code -ne 0 ]; then echo -e "\nThe Total Errors: $exit_code\n"; exit 1; else echo -e "GITLINT PASSED\n"; fi;
popd > /dev/null || exit
