...
class Vpn():
    ...
    def get_node(self):
        ...
        resp = requests.get(url=url, headers=self.headers, verify=False)  # type: Response
        if resp.status_code == 200:
            self.node = resp.json()

    def connect(self):
        if self.node:
            url = "https://localhost/api/kit/node/{}/vpn".format(self.node["hostname"])
            data = { "vpn_status": True }
            req = requests.post(url=url, json=data, headers=self.headers, verify=False)  # type: Response
    ...
    def disconnect(self):
        if self.node:
            ...
            req = requests.post(url=url, json=data, headers=self.headers, verify=False)  # type: Response
            if req.status_code == 200 or req.status_code == 201:
                print("{} disconnected from vpn.".format(self.node["hostname"]))
    ...
    def deploy_node(self):
        if self.node:
            url = "https://localhost/api/kit/node/{}/deploy".format(self.node["hostname"])
            req = requests.post(url=url, headers=self.headers, verify=False) # type: Response
            if req.status_code == 200 or req.status_code == 201:
                print("Starting deployment for {}.".format(self.node["hostname"]))

    def get_deploy_job(self):
        if self.node:
            self.deploy_job = next(job for job in self.node["jobs"] if job["name"] == "deploy")
...
def main():
    ...
    if args.state == "connect":
        ...
        if "inprogress" in vpn.deploy_job and vpn.deploy_job["inprogress"]:
            print("{} deploy job is in progress no need to start the deploy stage.".format(vpn.hostname))
            return
