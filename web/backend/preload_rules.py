...
class PreloadRuleSets():
    ...
    def create_ruleset(self, ruleset: dict):
        resp = requests.post("https://localhost/api/policy/ruleset", headers=self.headers, json=ruleset, verify=False)
    ...
    def upload(self, pathlist, ruleset_id):
        for path in pathlist:
            ...
            resp = requests.post("https://localhost/api/policy/rule/upload", data=data, files=files, headers=self.headers, verify=False)
            if resp.status_code == 200:
    ...
    def generate_api_key(self):
        ...
        self.headers = {"Authorization": "Bearer {}".format(stdout.decode('utf-8').strip())}
        return

    def get_rulesets(self):
        resp = requests.get("https://localhost/api/policy/ruleset", headers=self.headers, verify=False)
        if resp.status_code == 200:
    ...
    def delete_rulesets(self):
        self.get_rulesets()
        for ruleset in self.rulesets:
            if ruleset["name"] == "Emerging Threats" or ruleset["name"] == "Zeek Sample Scripts":
