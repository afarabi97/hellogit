...
COLD_LOG_CONFIGURE_WINLOGBEAT = (
    BACKEND_DIR + "playbooks/configure_cold_log_winlogbeat.yml"
)
COLD_LOG_INSTALL_WINLOGBEAT = (
    BACKEND_DIR + "playbooks/install_winlogbeat_for_cold_log_ingest.yml"
)
...
class WinrmCommandFailure(Exception):
    def __init__(self, message: str, results: Dict, results_list):
        super(WinrmCommandFailure, self).__init__(
            message,
        )
...
def execute_win_playbook(
    ...
    if status_code != 0:
        raise WinrmCommandFailure(
            "Failed with status_code {}. Run tail -f /var/log/tfplenum/rq.log for more information.".format(
                status_code
            ),
...
def reinstall_agent(
    ...
    agent_zip_path="agents.zip",
) -> Dict:
    print(
        "Reinstalling agent with hosts: {}, username: {}, port: {}, winrm_scheme: {}, winrm_transport: {}".format(
            str(hosts), username, port, ansible_winrm_scheme, winrm_transport
        )
    )
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        inventory_path = _create_inventoryfile(
            hosts,
            username,
            password,
            port,
        ...
        ret_val = execute_win_playbook(
            [REINSTALL_AGENT_PKG], extra_vars, inventory_path
        )
    return ret_val
...
def uninstall_agent(
    ...
    print(
        "Uninstalling agent with host: {}, username: {}, port: {}, winrm_scheme: {}, winrm_transport: {}".format(
            str(hosts), username, port, ansible_winrm_scheme, winrm_transport
        )
    )
    if isinstance(hosts, str):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        ret_val = execute_win_playbook(
            [UNINSTALL_AGENT_PKG], extra_vars, inventory_path
        )
    return ret_val
...
def configure_and_run_winlogbeat_for_cold_log_ingest(
    config: WinlogbeatInstallModel, cold_log_zip_path: str, tmp_dir: str
):
    hosts = config.windows_host
    if isinstance(hosts, str):
        hosts = [config.windows_host]
...
class WindowsConnectionManager:
    def __init__(
        ...
        if protocol == WINRM_PROTOCOLS[3]:
            ...
            self._smb_executer = SMBClient(
                self._host, username=username, password=password, port=self._port
            )
    ...
    def run_smb_command(
        ...
        if executable == "powershell.exe":
            ...
            args = "-noprofile -executionpolicy bypass -EncodedCommand {}".format(
                encoded_cmd.decode("UTF-8")
            )

        stdout, stderr, rc = self._smb_executer.run_executable(
            executable,
            arguments=args.format(cmd),
            run_elevated=True,
    ...
    def push_file(
        self, local_source_file: Union[str, Path], remote_dest_path: str
    ) -> None:
        ...
        if not source.exists() or not source.is_file():
            raise WinrmFileNotFound(
                "The {} either does not exist or is not a file.".format(source)
            )
        ...
        if self._protocol == WINRM_PROTOCOLS[3]:
            ...
            raise ProtocolNotSupported(
                "push_file not supported for {}".format(self._protocol)
            )

    def pull_file(self, remote_src: str, local_dest: str):
        if self._protocol == WINRM_PROTOCOLS[3]:
