...
def build_query_clause(acknowledged: str,
                       escalated: str,
                       start_time: str,
                       end_time: str,
                       search_kind: str,
    ...
    if acknowledged not in ('no','yes'):
        raise ValueError("Invalid value passed in for acknowledge it must be either yes or no")

    if escalated not in ('no','yes'):
        raise ValueError("Invalid value passed in for escalated it must be either yes or no")

    if show_closed not in ('no','yes'):
        raise ValueError("Invalid value passed in for show_closed it must be either yes or no")
...
class AlertsFieldsCtrl(Resource):
    ...
    def get(self) -> Response:
        ...
        fields = []
        default_fields = ("agent.*,as.*,bucket_name.*,client.*,code_signature.*,destination.*,dll.*,dns.*,ecs.*,error.*,"
        ...
        for ind in index_list:
            if elastic.indices.exists(index=ind["index"], allow_no_indices=False):
                payload = elastic.indices.get_field_mapping(fields=ind["fields"], index=ind["index"], doc_type=None, params=None, headers=None )
...
class AlertsDetailedCtrl(Resource):
    ...
    def post(self, count_override: int):
        ...
        payload = es.search(body=body, index=search_index, doc_type=None, params=None, headers=None, request_timeout=ELASTIC_TIMEOUT)
        return payload
...
class AlertsCtrlv2(Resource):
    ...
    def _get_table_contents(self, query_payload: Dict, fields: List[str], table_rows: List[Dict]):
    ...
    def get(self,
        ...
        for search_kind in self._SEARCH_KINDS:
            ...
            body["size"] = 0
            body["query"] = build_query_clause(acknowledged=acknowledged, escalated=escalated, start_time=start_time,
                                               end_time=end_time, search_kind=search_kind, show_closed=show_closed)
            try:
                payload = elastic.search(body=body, index=search_index, doc_type=None, params=None, headers=None, request_timeout=ELASTIC_TIMEOUT)
            except ConnectionTimeout as e:
                return {"message": "Elasticsearch query timed out after {} seconds.  To fix this, narrow the Window time by reducing the number of days / hours until the page refreshes successfully.".format(ELASTIC_TIMEOUT)}, 400
            except RequestError as e:
                body = self._fix_keyword_issue(e, fields_ary)
                body["size"] = 0
                body["query"] = build_query_clause(acknowledged=acknowledged, escalated=escalated, start_time=start_time,
                                                   end_time=end_time, search_kind=search_kind, show_closed=show_closed)
                payload = elastic.search(body=body, index=search_index, doc_type=None,
                                         params=None, headers=None, request_timeout=ELASTIC_TIMEOUT)
                ...
                if escalated == 'no':
                    ...
                    body["script"] = {
                        "source": '''
                            ...
                            ctx._source.event.hive_id = ''' + hive_id  + ''';
                            ctx._source.signal.status = "in-progress";
                        ''',
                        "lang": "painless"
                    }
                    ret_val = elastic.update_by_query(index=indicies, body=body, request_timeout=ELASTIC_TIMEOUT)
            else:
                if acknowledged == 'yes':
                    body["script"] = {
                        "source": '''
...
def webhook():
    ...
    try:
        event_kind = event['object']['customFields']['event.kind']["string"] if event['object']['customFields']['event.kind']["string"] else 'alert'
    except KeyError:
        pass
    ...
    try:
        hive_impact_status = event['details']['impactStatus'] if event['details']['impactStatus'] else ''
    except KeyError:
        pass

    try:
        hive_resolution_status = event['details']['resolutionStatus'] if event['details']['resolutionStatus'] else ''
    except KeyError:
        pass
    ...
    body["script"] = {
        "source": '''
            ctx._source.event.hive_resolution_status = "''' + hive_resolution_status + '''";
            ctx._source.event.hive_status = "''' + hive_status + '''";
            ctx._source.event.hive_impact_status = "''' + hive_impact_status + '''";
        ''',
        "lang": "painless"
    }
