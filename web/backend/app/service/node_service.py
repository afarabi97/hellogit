...
def _execute_job(cmd_object: Command) -> bool:
    async_job = AsyncJob(
        job_name=cmd_object.job_name,
        job_id=cmd_object.job_id,
        command=cmd_object.command,
        working_dir=cmd_object.cwd_dir,
        use_shell=True,
    )
...
def send_notification() -> None:
    notify_node_management(
        kit_status=get_kit_status(), node_data=get_all_nodes_with_jobs()
    )


@job("default", connection=REDIS_CLIENT, timeout="5m")
def update_device_facts_job(
    node: Node, settings: Union[KitSettingsForm, MipSettingsForm]
) -> None:
    ...
    try:
        ...
        device_facts = create_device_facts_from_ansible_setup(
            str(node.ip_address), settings.password
        )
        if device_facts:
            node.deviceFacts = device_facts.to_dict()
            node.save_to_db()
            notification.set_and_send(
                message="{} device facts updated.".format(node.hostname),
        ...
        notification.set_and_send(
            message="Unable to update {} device facts.".format(node.hostname),
...
def gather_device_facts(node: Node, settings: Union[KitSettingsForm, MipSettingsForm]):
    ...
    try:
        ...
        node_job = NodeJob.load_job_by_node(
            node=node, job_name=JOB_PROVISION
        )  # type: NodeJob
        if node_job:
        ...
        while True:
            ...
            if device_facts:
                ...
                if node_job:
                    ...
                    node_job.set_complete()
                    log_text = "INFO:{}: {} successfully provisioned.".format(
                ...
                if node.node_type == NODE_TYPES.mip.value:
                    ...
                    node.node_type == NODE_TYPES.control_plane.value
                    and node.deployment_type == DEPLOYMENT_TYPES.baremetal.value
                ):
                    execute.delay(
                        node=node,
                ...
                sleep(10)
                log_text = "INFO:{}: Waiting for node {} to provision - This can take a while, you may want to grab a coffee".format(
                    str(datetime.now()), node.hostname
                )
...
class NodeService:
    ...
    def get_cmd(self) -> str:
        ...
        if self.exec_type == DEPLOYMENT_JOBS.base_kit:
            ...
            self.cmd = (
                "export ANSIBLE_LOG_PATH={log_path}/core.log;"
                "ansible-playbook site.yml -i inventory {skip_tags}".format(
                    log_path=LOG_PATH, skip_tags=skip_tags
                )
            )
        # Add Node
            ...
            self.cmd = (
                ...
                "--limit localhost,{node} {skip_tags}".format(
                    log_path=LOG_PATH, node=self.node.hostname, skip_tags=skip_tags
                )
            )
        # Remove Node
        ...
        # Control Plane
            ...
            self.exec_type == DEPLOYMENT_JOBS.setup_control_plane
            and self.stage == JOB_DEPLOY
        ):
            self.cmd = (
                "export ANSIBLE_LOG_PATH={log_path}/control-plane.log;"
                "ansible-playbook control_plane_playbook.yml -i inventory -t deploy-control-plane"
            ).format(log_path=LOG_PATH)

        elif self.exec_type == DEPLOYMENT_JOBS.provision_virtual or (
            self.exec_type == DEPLOYMENT_JOBS.setup_control_plane
            and self.stage == JOB_PROVISION
        ):
            self.cmd = (
                "export ANSIBLE_LOG_PATH={log_path}/virtual-node.log;"
                "ansible-playbook virtual_node_playbook.yml -i inventory -t provision-virtual-machine "
                "-i {mip_dir} -e node='{node}'"
            ).format(
                log_path=LOG_PATH,
        ...
        # Kickstart Profiles
            ...
            self.cmd = (
                "export ANSIBLE_LOG_PATH={log_path}/kickstart-profiles.log;"
        ...
        # Setup Controller Run on Save General Settings
            ...
            ).format(log_path=LOG_PATH, mip_dir=str(MIP_DIR / "inventory"))
        ...
        # Save Kit Settings
            ...
            self.cmd = (
                "export ANSIBLE_LOG_PATH={log_path}/kit-setting.log;"
                "ansible-playbook controller_playbook.yml -i inventory -i {mip_dir} "
                "-t create-assessor-user,setup_controller_kit_settings"
            ).format(log_path=LOG_PATH, mip_dir=str(MIP_DIR / "inventory"))
    ...
    def handle_success(self) -> None:
        notification = NotificationMessage(role=_JOB_NAME_NOTIFICATION.lower())
        msg = "{} {} job successfully completed.".format(
            self.exec_type.value, self.stage
        )
        if isinstance(self.node, list):
            ...
            msg = "{} {} job successfully completed on node '{}'.".format(
        ...
        if self.exec_type == DEPLOYMENT_JOBS.setup_control_plane:
            if self.stage == JOB_CREATE:
                execute.delay(
                    node=self.node, exec_type=self.exec_type, stage=JOB_PROVISION
                )
...
def execute(
    ...
    try:
        ...
        node_service = NodeService(
            job_id=job_id, exec_type=exec_type, stage=stage, node=node
        )  # type: NodeService
        if node:
        ...
        if (
            exec_type == DEPLOYMENT_JOBS.mip_deploy
            and node.node_type == NODE_TYPES.mip.value
        ):
            dir = str(MIP_DIR)
        else:
        ...
        rq_logger.info(dir)
        command = Command(
            command=cmd, cwd_dir=dir, job_name=_JOB_NAME_NOTIFICATION, job_id=job_id
        )
        ...
        if node_service:
            ...
            success = False
            log_to_console(
                _JOB_NAME_NOTIFICATION,
                node_service.job_id,
                "An exception occurred: {}".format(str(exc)),
                "red",
            )
        raise
    return False
...
def refresh_kit(nodes: List[Node], new_control_plane: List[Node]):
    ...
    notification = NotificationMessage(role=_JOB_NAME_NOTIFICATION.lower())
    notification.set_and_send(
        message="Kit Refresh started.", status=NotificationCode.STARTED.name
    )
    ...
    for node in nodes:
        if node.node_type == NODE_TYPES.sensor.value:
            ...
            if node.deployment_type == DEPLOYMENT_TYPES.baremetal.value:
                results.append(
                    execute.delay(
                        exec_type=DEPLOYMENT_JOBS.kickstart_profiles, node=node
                    )
                )
    ...
    results.append(
        execute.delay(
            node=new_control_plane[0], exec_type=DEPLOYMENT_JOBS.setup_control_plane
        )
    )
