...
class VpnService:
    ...
    def sign_request(self):
        stdout, rtn_code = run_command2(
            working_dir=WORKING_DIR,
            command="./easyrsa --batch --req-cn={short_hostname} sign-req client {short_hostname} ".format(
    ...
    def generate_key_request(self):
        stdout, rtn_code = run_command2(
            working_dir=WORKING_DIR,
            command="./easyrsa --batch --req-cn={short_hostname} gen-req {short_hostname} nopass".format(
                short_hostname=self.short_hostname
            ),
            use_shell=True,
        )
    ...
    def generate_client_file(self):
        filename = "{}/server/client-scripts/{}".format(
            WORKING_DIR, self.short_hostname
        )
        with open(filename, "w") as f:
            f.write(
                "ifconfig-push {} {}".format(
                    self.node.ip_address, self.kitSettings.netmask
                )
            )
            f.write("\r\n")

    def generate_config(self) -> str:
        self.short_hostname = self.node.hostname.replace(
            ".{domain}".format(domain=self.kitSettings.domain), ""
        )
