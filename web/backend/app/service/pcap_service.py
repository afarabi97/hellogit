...
def replay_pcap_using_tcpreplay(payload: Dict, root_password: str):
    ...
    try:
        ...
        with FabricConnectionManager('root', root_password, replay.sensor_ip) as ssh_con:
...
def _save_all_configmap_data(search_dict: Dict, namespace: str, config_name: str, some_dir: str):
    for i in search_dict['items']:
        if i['metadata']['namespace'] == namespace and config_name in i['metadata']['name']:
...
class SuricataReplayer:
    ...
    def pull_suricata_rules(self):
        ...
        with FabricConnectionManager('root', self.kit_password, self.replay.sensor_ip) as ssh_con:
    ...
    def run_suricata_docker_cmd(self, pcap: Path) -> int:
        pcap_name = pcap.name
        cmd = ("docker run --rm -v {pcap_path}:/data/pcap/{pcap_name} "
               ...
               "-r /data/pcap/{pcap_name} -l /logs").format(pcap_path=str(pcap),
                                                            pcap_name=pcap_name,
    ...
    def _is_installed(self):
        applications = [ i['application'] for i in get_node_apps(self.replay.sensor_hostname) ]
    ...
    def execute(self, pcap: Path):
        ...
        try:
            ...
            if self.run_suricata_docker_cmd(pcap) != 0:
                raise Exception("Failed to replay the PCAP using suricata. View /var/log/tfplenum/rq.log for more details.")
...
class ZeekReplayer:
    ...
    def run_zeek_docker_cmd(self, pcap: Path) -> int:
        pcap_name = pcap.name
        cmd = ("docker run -v {tmp_dir}/zeek/intel.dat:/opt/tfplenum/zeek/intel.dat "
               ...
               "-r /pcaps/{pcap_name} "
               "/opt/zeek/share/zeek/site/local.zeek").format(tmp_dir=self.tmp_dir,
                                                              pcap_path=str(pcap),
                                                              pcap_name=pcap_name,
    ...
    def execute(self, pcap: Path):
        ...
        try:
            ...
            if self.run_zeek_docker_cmd(pcap) != 0:
                raise Exception("Failed to replay the PCAP using zeek. View /var/log/tfplenum/rq.log for more details.")
            self.push_logs_files()
        finally:
            shutil.rmtree(self.tmp_dir)
...
class ArkimeReplayer:
    ...
    def _get_arkime_config_data(self, search_dict: Dict):
        for i in search_dict['items']:
            if i['metadata']['namespace'] == "default" and "arkime" in i['metadata']['name']:
    ...
    def run_arkime_docker_cmd(self, pcap: Path):
        pcap_name = pcap.name
        cmd = ("docker run --rm -v {pcap_path}:/pcaps/{pcap_name} "
               ...
               "-r /pcaps/{pcap_name}").format(pcap_path=str(pcap),
                                               pcap_name=pcap_name,
                                               tmp_dir=self.tmp_dir,
    ...
    def patch_configuation(self):
        ...
        lines[line_index_to_change] = "elasticsearch=https://arkime:password@tfplenum-es-data-0.tfplenum-es-data.default.svc.cluster.local:9200\n"
    ...
    def execute(self, pcap: Path):
        ...
        try:
            ...
            if self.run_arkime_docker_cmd(pcap) != 0:
                raise Exception("Failed to replay the PCAP using Arkime. View /var/log/tfplenum/rq.log for more details.")
        finally:
            shutil.rmtree(self.tmp_dir)
...
def replay_pcap_using_preserve_timestamp(payload: Dict, root_password: str):
    ...
    try:
        ...
        if not pcap.exists() or not pcap.is_file():
            notification.set_status(status=NotificationCode.ERROR.name)
            notification.set_message("{} does not exist or is not a file.".format(str(pcap)))
            notification.post_to_websocket_api()
