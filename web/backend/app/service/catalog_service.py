...
def _get_elastic_nodes(node_type) -> list:
    ...
    with KubernetesWrapper() as kube_apiv1:
        try:
            api_response = kube_apiv1.list_pod_for_all_namespaces(
                watch=False, label_selector=label_selector
            )
            for node in api_response.items:
                name = node.metadata.name
                stateful_set = node.metadata.labels[
                    "elasticsearch.k8s.elastic.co/statefulset-name"
                ]
                nodes.append(
                    "https://{name}.{set}.default.svc.cluster.local:9200".format(
                        name=name, set=stateful_set
                    )
                )
    ...
    with KubernetesWrapper() as kube_apiv1:
        try:
            ...
            for node in api_response.items:
                ...
                nodes.append(
                    "{name}.{set}.default.svc.cluster.local:5050".format(
                        name=name, set=stateful_set
                    )
                )
        except Exception as exc:
...
def get_node_apps(node_hostname: str) -> list:
    ...
    if node.node_type == NODE_TYPES.sensor.value:
        ...
        node.node_type == NODE_TYPES.service_node.value
            ...
            if application:
                for chart in value:
                    if chart.get("annotations"):
                        ...
                        t_chart["pmoSupported"] = _to_bool(
                            chart["annotations"]["pmoSupported"]
                        )
                        t_chart["isSensorApp"] = _to_bool(
                            chart["annotations"]["isSensorApp"]
                        )
...
def get_app_state(
    ...
    try:
        ...
        current_charts = [
            chart for chart in chart_releases if re.match(regex, chart["name"])
        ]
        for chart in current_charts:
            ...
            filter_sv = [
                svalues
                for svalues in saved_values
                if svalues["deployment_name"] == chart["name"]
            ]
            if filter_sv:
...
def install_helm_apps(
    application: str, namespace: str, node_affinity: str, values: list, task_id=None
):
    get_app_context().push()
    response = []
    deployment_names = []
    notification = NotificationMessage(
        role=_MESSAGETYPE_PREFIX,
        action=NotificationCode.INSTALLING.name.capitalize(),
    ...
    for value in values:
        ...
        message = "%s %s" % (
            NotificationCode.INSTALLING.name.capitalize(),
        ...
        if deployment_name and value_items:
            ...
            try:
                ...
                mongo_catalog_saved_values().delete_one(
                    {"application": application, "deployment_name": deployment_name}
                )
                mongo_catalog_saved_values().insert(
                    {
                        "application": application,
                        "deployment_name": deployment_name,
                        "values": value_items,
...
@job("default", connection=REDIS_CLIENT, timeout="30m")
def install_helm_command(
    deployment_name: str, application: str, node: str, namespace: str, value_items: dict
):
    get_app_context().push()
    response = []
    if not _check_chart_name(application):
    ...
    if ret_code == 0 and stdout != "":
        ...
        response.append(
            'release: "' + deployment_name + '" ' + results["STATUS"].upper()
        )
    if ret_code != 0 and stdout != "":
...
def delete_helm_apps(application: str, namespace: str, nodes: List, from_helm_delete_ctrl=True):
    ...
    notification = NotificationMessage(
        role=_MESSAGETYPE_PREFIX,
        action=NotificationCode.UNINSTALLING.name.capitalize(),
        application=application.capitalize(),
    )
    response = []
    deployment_names = []
    for node in nodes:
        ...
        if deployment_to_uninstall:
            deployment_names.append(deployment_name)
            message = "%s %s on %s" % (
                NotificationCode.UNINSTALLING.name.capitalize(),
                application,
                node_hostname,
            )
            notification.set_message(message=message)
            ...
            try:
                ...
                if node_hostname:
                    execute_kubelet_cmd(
                        "kubectl label nodes " + node_hostname + " " + application + "-"
                    )
                delete_helm_command.delay(
    ...
    # Send Update Notification to websocket
    while any(
        item in deployment_names
        for item in [
            d["deployment_name"]
            for d in get_app_state(application=application, namespace=namespace)
        ]
    ):
        sleep(0.25)
...
@job("default", connection=REDIS_CLIENT, timeout="30m")
def delete_helm_command(
    deployment_name: str, application: str, node: str, namespace: str
):
    get_app_context().push()
    response = []
    stdout, ret_code = run_command2(
