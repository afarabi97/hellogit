...
@celery.task
def perform_add_node(command: str):
    notification = NotificationMessage(role=_JOB_NAME_NOTIFICATION)
    notification.setMessage("%s started." % _JOB_NAME_NOTIFICATION.capitalize())
    notification.setStatus(NotificationCode.STARTED.name)
    notification.post_to_websocket_api()

    cwd_dir = str(CORE_DIR / "playbooks")
    job = AsyncJob(_JOB_NAME, command, working_dir=cwd_dir)

    notification.setMessage("%s in progress." % _JOB_NAME_NOTIFICATION.capitalize())
...
# @celery.task
# def perform_pre_add_node(command: str):
#     cwd_dir = str(CORE_DIR / "playbooks")
#     job = AsyncJob(_JOB_NAME_ONE.capitalize(), command, working_dir=cwd_dir)
#     ret_val = job.run_asycn_command()
#     conn_mng.mongo_celery_tasks.delete_one({"_id": _JOB_NAME_ONE.capitalize()})
#     return ret_val

# @celery.task
# def perform_add_node(command: str):
#     cwd_dir = str(CORE_DIR / "playbooks")
#     job = AsyncJob(_JOB_NAME_TWO.capitalize(), command, working_dir=cwd_dir)
#     ret_val = job.run_asycn_command()
#     conn_mng.mongo_celery_tasks.delete_one({"_id": _JOB_NAME_TWO.capitalize()})
#     return ret_val
