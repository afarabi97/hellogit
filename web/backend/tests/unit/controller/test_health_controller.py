...
from tests.unit.static_data import zeek_stats, nodes_and_pods
...
def test_suricata_installed(client, mocker):
    ...
    mocker.patch(
        "app.controller.health_controller.get_suricata_stats",
        return_value=(0,0)
    )
...
def test_zeek_installed(client, mocker):
    ...
    mocker.patch(
        "app.controller.health_controller.get_zeek_stats",
        return_value=zeek_stats
    )
