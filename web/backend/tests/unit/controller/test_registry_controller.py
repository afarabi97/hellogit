...
from tests.unit.static_data.docker_registry import mock_docker
...
def test_get_docker_registry_status_200(client: FlaskClient, mocker: MockerFixture) -> None:
    mocker.patch("app.controller.registry_controller.get_docker_registry", return_value=mock_docker[0])
    response = client.get("api/kubernetes/docker/registry")
    assert response.status_code == 200
    assert json_object_key_value_checker(response.json, mock_docker[0]) == True
