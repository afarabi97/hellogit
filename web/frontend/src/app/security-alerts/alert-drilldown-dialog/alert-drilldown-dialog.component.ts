...
@Component({
  ...
  styleUrls: ['alert-drilldown-dialog.component.css'],
})
export class AlertDrillDownDialog implements OnInit {
  dynamicColumns: string[];
  allColumns: string[];
  alerts: MatTableDataSource<Object>;
  links: object[];

  @ViewChild('dialogPaginator')
  dialogPaginator: MatPaginator;

  @ViewChild(MatSort)
  sort: MatSort;

  constructor( public dialogRef: MatDialogRef<AlertDrillDownDialog>,
  ...
  private setColumns(alert: Object){
    //{count: 2590, rule.name: "ET EXPLOIT Possible DOUBLEPULSAR Beacon Response", destination.address: "192.168.116.149", form: {…}}
    this.dynamicColumns = [];
  ...
  openKibana(alert: Object) {
    ...
    if (alert['_source']['event']['kind'] && alert['_source']['event']['kind'] === "signal") {
      url = `${prefix}/app/security/detections?query=
(language:kuery,query:%27_id%20:%20%22${id}%22%27)
&timerange=(global:(linkTo:!(timeline),timerange:
(from:%27${startDatetime}%27,kind:absolute,to:%27${endDatetime}%27)),
